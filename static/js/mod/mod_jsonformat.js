...
Date.prototype.format = function (e) {
    ...
    var f = this.getFullYear(), d = this.getMonth() + 1, i = this.getDate(), g = this.getHours(), c = this.getMinutes(),
        h = this.getSeconds();
    b(/yyyy/g, a(f, 4));
...
var JsonFormatEntrance = (function () {
    var s, p, o, i, g, k = JsonFormatDealer, m = +(new Date()), r, v, b, w;
    var j = function (C) {
        switch (C[0]) {
            case"NOT JSON":
                p.style.display = "";
            ...
            case"FORMATTING":
                ...
                z.id = "optionBar";
                var B = document.createElement("button"), y = document.createElement("button");
    ...
    function n(B) {
        if (B.which === 1) {
            ...
            if (A.className === "e") {
                B.preventDefault();
                var z = A.parentNode, F = s, y = document.body.offsetHeight, D = document.body.scrollTop, C;
                    ...
                    A.root.getFile(D, {create: true}, function (F) {
                        F.createWriter(function (J) {
                            J.onwriteend = function () {
                                $("#optionBar").prepend('<a href="' + F.toURL() + '" id="btnDownload" target="_blank" title="在新页面Ctrl+S保存到本地">下载JSON数据</a>')
                            };
                            var H = location.href;
...
baidu.jsonformat = (function () {
    ...
    var c = function (h) {
        var i = function (k) {
            var j = h.text().replace(/":\s/gm, '":').replace(/,$/, "").trim();
        ...
        f.find("a.opt-copy").unbind("click").bind("click", i);
        f.find("a.opt-del").unbind("click").bind("click", g);
    ...
    var d = function () {
        ...
        $("#jfContent").delegate(".kvov", "click", function (g) {
            ...
            var f = $(this).removeClass("x-hover").addClass("x-outline");
            c(f);
            if (!$(g.target).is(".kvov .e")) {
                g.stopPropagation()
            } else {
                $(g.target).parent().trigger("click")
            }
    ...
    var e = function () {
        ...
        //     if (g.type == MSG_TYPE.TAB_CREATED_OR_UPDATED && g.event == "jsonformat") {
        //         if (g.content) {
        //             document.getElementById("jsonSource").value = (g.content);
        //             a()
        //         }
        //     }
        // });
